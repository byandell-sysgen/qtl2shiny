...
#' @export
#' @rdname scanDataApp
scanDataServer <- function(id, hotspot_list, snp_list, probs_obj, project_df) {
  shiny::moduleServer(id, function(input, output, session) {
    ...
    scan_obj <- shiny::reactive({
      shiny::req(
        hotspot_list$pheno_mx(), probs_obj(),
    ...
    scan_plot <- shiny::reactive({
      ...
      shiny::req(win_par(), scan_window(), scan_obj(), probs_obj())
    ...
    eff_obj <- shiny::reactive({
      ...
      shiny::withProgress(message = "Effect scans ...", value = 0, {
        shiny::setProgress(1)
        scan1_effect(
          probs_obj()$probs, hotspot_list$pheno_mx(),
          hotspot_list$kinship_list(), hotspot_list$covar_df(),
    ...
    coef_plot <- shiny::reactive({
      ...
      shiny::withProgress(message = "Effect plots ...", value = 0, {
        shiny::setProgress(1)
        plot_eff(
          pheno_name(), eff_obj(), map, scan_obj(),
          scan_window(), , hotspot_list$allele_info()
        )
      })
    })
    ...
    download_Filename <- shiny::reactive({
      ...
      c(
        Plot = paste(out,
          ifelse(scan_tab == "Summary", "Both", scan_tab),
          sep = "_"
        ),
