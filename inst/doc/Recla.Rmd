...
```{r}
snp_scan_obj <- scan1covar(pheno_mx, covar_df, snpprobs_obj$snpprobs,
                           kinship_list, peak_df)
```
...
```{r}
(snp_pattern_table <- dplyr::mutate(summary(top_snps_tbl),
...
```{r}
scan_pat <- qtl2shiny:::scan1_pattern(pheno_name, pheno_mx, covar_df,
  pair_probs_obj, kinship_list, peak_df, pats, blups = FALSE)
```

```{r}
(pattern_table <- dplyr::mutate(summary(scan_pat, pairprobs_obj$map),
...
pattern_choices <- qtl2pattern::sdp_to_pattern(pats$sdp, haplos)
```

[patternPlotServer()](https://github.com/byandell-sysgen/qtl2shiny/blob/master/R/patternPlotApp.R)

```{r}
qtl2shiny:::scan_pat_type(scan_pat, pair_probs_obj$map, "lod",
                          pattern_choices, pheno_name, haplos)
```

```{r}
qtl2shiny:::scan_pat_type(scan_pat, pair_probs_obj$map, "coef",
                          pattern_choices, pheno_name, haplos)
```
